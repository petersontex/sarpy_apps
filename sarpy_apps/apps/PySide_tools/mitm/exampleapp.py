...
class Controller(AbstractApp):
    ...
    def change_appearance(self, dark_mode):
        ...
        return super().change_appearance(dark_mode)

    def load(self):
        super().load()
        print("loading ", self.get_name())  # debugging

        # setup coordinates stuff
        self.viewer.button.clicked.connect(self.check_coordinates)
        self.set_plot_widget(self.mitm_controller.get_current_widget())
        self.mitm_controller.viewer.widget_changed_signal.connect(self.set_plot_widget)

        # setup color change stuff
        target_id = self.model.get_target_id()
        self.viewer.button.clicked.connect(
            self.app_dict["My Skeleton App " + str(target_id)].change_button_color
        )

    def offload(self):
        super().offload()
        print("offloading ", self.get_name())  # debugging

        # teardown coordinates stuff
        self.viewer.button.clicked.disconnect(self.check_coordinates)
        self.mitm_controller.viewer.widget_changed_signal.disconnect(
            self.set_plot_widget
        )

        # teardown color change stuff
        target_id = self.model.get_target_id()
        self.viewer.button.clicked.disconnect(
            self.app_dict["My Skeleton App " + str(target_id)].change_button_color
        )
    ...
    def change_button_color(self):
        ...
        # slots for signals from other apps or mitm_controller
        # should check if this app is loaded before performing operations
        if self.is_loaded:
            button = self.viewer.button
    ...
    def check_coordinates(self):
        ...
        if self.model.plotWidget:
            if self.model.plotWidget.coordinatesButton.text():
                self.viewer.button.setText(
                    self.model.plotWidget.coordinatesButton.text()
                )


class View(QDockWidget):
    """
    The View class for ExampleApp. Should typically inherit from QDockWidget.
...
from exampleapp import Controller as ExampleAppController
...
if __name__ == "__main__":
    ...
    controller = WrapperController(
        [ExampleAppController(1), ExampleAppController(2), ExampleAppController(3)]
    )
