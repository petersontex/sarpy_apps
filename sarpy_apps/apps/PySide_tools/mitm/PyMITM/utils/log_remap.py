...
def clip_cast(
    ...
    min_value = (
        numpy.iinfo(np_type).min
        if min_value is None
        else max(min_value, numpy.iinfo(np_type).min)
    )
    max_value = (
        numpy.iinfo(np_type).max
        if max_value is None
        else min(max_value, numpy.iinfo(np_type).max)
    )
...
class RemapFunction(object):
    ...
    def _set_bit_depth(self, value: int):
        ...
        if value not in [8, 16, 32]:
            raise ValueError(
                "Bit depth is required to be one of 8, 16, or 32 and we got `{}`".format(
                    value
                )
            )
    ...
    def _set_dimension(self, value: int):
        ...
        if self._allowed_dimension is not None and value not in self._allowed_dimension:
            raise ValueError(
                "Dimension is required to be one of `{}`, got `{}`".format(
                    self._allowed_dimension, value
                )
            )
    ...
    def raw_call(self, data: numpy.ndarray, **kwargs) -> numpy.ndarray:
        """
    ...
    def call(self, data: numpy.ndarray, **kwargs) -> numpy.ndarray:
        """
    ...
    def _validate_pixel_bounds(
        ...
        if not (
            (-data_size[0] <= pixel_bounds[0] <= data_size[0])
            and (-data_size[0] <= pixel_bounds[1] <= data_size[0])
            and (-data_size[1] <= pixel_bounds[2] <= data_size[1])
            and (-data_size[1] <= pixel_bounds[3] <= data_size[1])
        ):
            raise ValueError(
                "invalid pixel bounds `{}` for data of shape `{}`".format(
                    pixel_bounds, data_size
                )
            )
        return pixel_bounds
...
class MonochromaticRemap(RemapFunction):
    ...
    def __init__(
        ...
        RemapFunction.__init__(
            self, override_name=override_name, bit_depth=bit_depth, dimension=0
        )
    ...
    def calculate_global_parameters_from_reader(
        self, reader, index=0, pixel_bounds=None
    ):
        raise NotImplementedError
    ...
    def raw_call(
        ...
        if numpy.any(use_mask):
            ...
            if min_value == max_value:
                ...
                rcent = 10 * numpy.log10(
                    numpy.sum(numpy.square(temp_data)) / temp_data.size
                )
